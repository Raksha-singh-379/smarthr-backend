...
type RestOf<T extends IDocument | NewDocument> = Omit<T, 'lastModified'> & {
...
export class DocumentService {
  ...
  protected convertDateFromClient<T extends IDocument | NewDocument | PartialUpdateDocument>(document: T): RestOf<T> {
    return {
      ...document,
      lastModified: document.lastModified?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDocument: RestDocument): IDocument {
    return {
      ...restDocument,
      lastModified: restDocument.lastModified ? dayjs(restDocument.lastModified) : undefined,
    };
  }
